...
function groupMappingModal() {
  openModal({ kicker: "TENANT · OIDC GROUP", title: "OIDC Group Mapping 추가", body: `<div class="step-mini"><b>1 Group</b><span>2 Tenant Role</span><span>3 Scope</span><span>4 Review</span></div><label class="modal-field"><span>OIDC issuer</span><select><option>Keycloak · https://keycloak.example.com/realms/aiops</option></select></label>${formField("Group claim value", "/companies/aa/operators", "Token의 groups claim과 정확히 일치해야 합니다.")}<div class="field-grid modal-grid"><label>Tenant<select><option>AA Company</option><option>Platform Engineering</option></select></label><label>Role<select><option>Operator</option><option>Cluster Admin</option><option>Tenant Admin</option><option>Viewer</option></select></label><label>Scope type<select><option>Tenant 전체</option><option>Workspace</option><option>Cluster</option><option>Namespace</option></select></label><label>Effective access<input value="Applications · Analysis · Console" disabled /></label></div><div class="warning-box"><strong>Group 이름만으로 권한을 추측하지 않습니다.</strong><p>이 Mapping은 AA Company 안에서만 유효하며 다른 Tenant에는 적용되지 않습니다.</p></div><label class="external-consent"><input type="checkbox" data-require-check /><span>issuer, Group, Tenant, Role과 Scope를 확인했습니다.</span></label>`, confirm: "Mapping 저장", onConfirm: () => toast("OIDC Group Mapping을 저장하고 access revision을 갱신했습니다.") });
}
...
function actionFor(button) {
  ...
  const actions = {
    ...
    "＋ OIDC Group Mapping": groupMappingModal,
