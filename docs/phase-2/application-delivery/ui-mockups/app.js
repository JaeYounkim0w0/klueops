...
function actionFor(button) {
  ...
  const actions = {
    ...
    "확정 단계로 →": () => openModal({ kicker: "위험 작업 확인", title: "payments-web을 배포할까요?", body: exactBody("prod-seoul / payments에 6개 Kubernetes resource를 적용합니다.", "payments-web", "4개 생성, 2개 변경이 발생합니다. 실패 시 Helm atomic rollback 정책을 적용합니다."), confirm: "배포 실행", danger: true, exact: "payments-web", onConfirm: () => jobResult("Helm deployment") }),
    "＋ 새 배포": () => showScreen("discover"),
    "···": () => panel === "releases" ? openModal({ title: "Release 작업", body: `<div class="action-list"><button type="button">Release 상세 보기</button><button type="button">Audit 기록 보기</button><button type="button">Uninstall 계획 생성</button></div>`, confirm: "닫기" }) : simpleInfo("Provider 작업", "연결 검사, Profile 편집, 비활성화와 삭제 작업을 선택할 수 있습니다."),
