...
## 비-HTTP Service 음수 수용시험

PostgreSQL Chart `0.20.5`(App `18.6.0`)의 `postgres:5432`, `postgres-headless:5432`를 대상으로 같은 HTTPRoute 흐름을 점검했다. 두 Service는 렌더 결과에서 `HTTPRoute 사용 불가`로 비활성화되고 Preview 버튼도 활성화되지 않았다. Application 상세의 Service 주소는 `http://` 링크가 아니라 `tcp://...:5432`와 `전용 Client 또는 port-forward로 접속` 안내로 표시됐다.

기존에 잘못 생성된 `postgres-klueops` HTTPRoute는 Gateway listener의 `allowedRoutes.namespaces.from: Same` 정책 때문에 `Accepted=False`, `NotAllowedByListeners`로 `DEGRADED`였다. 이 상태는 유효한 접근 경로가 아니며 화면에서도 브라우저 링크로 제공하지 않는다. PostgreSQL 프로토콜은 HTTP가 아니므로 listener 정책만 `All`로 바꿔도 해결되지 않는다.

실제 데이터베이스 연결은 `kubectl -n docker-desktop-postgres port-forward service/postgres 15432:5432` 후 PostgreSQL client에서 `host=127.0.0.1 port=15432 dbname=postgres sslmode=disable`로 실행했다. `current_database=postgres`, `current_user=postgres`, PostgreSQL `18.6` 응답을 확인했다.
