...
- 안내는 `왼쪽`, `오른쪽`, `상단`처럼 배치에 의존하는 표현보다 업무 목적, 화면 이름, 대상 범위와 행동 이름을 사용한다.
- 주요 인증 화면은 실제 OIDC 로그인과 동작 확인 후 캡처하고 개인 계정, 실제 cluster 식별자와 내부 주소를 공개 가능한 예시 값으로 바꾼다.
- 화면별로 목적, 필요 권한, 주요 작업, 상태 해석과 실패 시 확인 항목을 함께 설명한다.
- 메뉴 최초 화면만 기록하지 않는다. 독립 상세 페이지, 탭, 팝업, Wizard와 확인 단계가 있으면 시작 조건부터 완료·취소·실패 기준까지 같은 업무 흐름으로 캡처하고 설명한다.
- 표는 비교가 필요한 기능·권한·상태에만 사용하며, 열 너비, 셀 여백, 반복 header, 교차 행 배경과 읽을 수 있는 글꼴 크기를 일관되게 적용한다.
