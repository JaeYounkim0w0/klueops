...
`.github/workflows/signed-container-release.yml`은 Backend, Frontend, Managed Keycloak, Command Runner 이미지를 동일한 절차로 처리한다. 일반 pull request와 `main`에서는 `.github/workflows/supply-chain.yml`이 publish 없이 동일한 네 이미지를 빌드하고 취약점을 검사한다.

1. 이미지를 빌드한다.
2. Trivy가 HIGH/CRITICAL 취약점을 차단한다.
3. Anchore Syft가 SPDX JSON SBOM을 생성한다.
...
- `global.productionMode=true`에서는 네 컴포넌트 image digest가 모두 필수다.
