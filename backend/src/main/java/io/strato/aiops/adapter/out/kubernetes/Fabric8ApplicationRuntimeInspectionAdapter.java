...
public class Fabric8ApplicationRuntimeInspectionAdapter implements ApplicationRuntimeInspectionPort {
    ...
    /** Fabric8ApplicationRuntimeInspectionAdapter 인스턴스를 필요한 의존성과 초기 상태로 구성한다. */
    ...
    /** Fabric8ApplicationRuntimeInspectionAdapter의 inspect 처리에 필요한 업무 로직을 수행한다. */
    @Override
    ...
    /** Fabric8ApplicationRuntimeInspectionAdapter의 httpRoutes 처리에 필요한 업무 로직을 수행한다. */
    ...
    /** Fabric8ApplicationRuntimeInspectionAdapter의 appendEndpoints 처리에 필요한 업무 로직을 수행한다. */
    private void appendEndpoints(String namespace, Service service, List<Endpoint> target) {
        ...
        service.getSpec().getPorts().forEach(port -> {
            String scheme = ServiceEndpointProtocol.scheme(port.getProtocol(), port.getAppProtocol(),
                    port.getName(), port.getPort());
            String name = service.getMetadata().getName();
            String serviceType = service.getSpec().getType() == null ? "ClusterIP" : service.getSpec().getType();
            String accessScope = switch (serviceType) {
            ...
            target.add(new Endpoint("CLUSTER_SERVICE", name,
                    ...
                    targetPort(port.getTargetPort()), port.getNodePort(), accessScope));
        });
    }

    /** Fabric8ApplicationRuntimeInspectionAdapter의 targetPort 처리에 필요한 업무 로직을 수행한다. */
    private String targetPort(io.fabric8.kubernetes.api.model.IntOrString targetPort) {
        if (targetPort == null) return null;
        if (targetPort.getStrVal() != null && !targetPort.getStrVal().isBlank()) return targetPort.getStrVal();
        return targetPort.getIntVal() == null ? null : targetPort.getIntVal().toString();
    }

    /** Fabric8ApplicationRuntimeInspectionAdapter의 ready 처리 결과를 조회해 반환한다. */
    ...
    /** Fabric8ApplicationRuntimeInspectionAdapter의 value 처리에 필요한 업무 로직을 수행한다. */
    private int value(Integer value) { return value == null ? 0 : value; }
    /** Fabric8ApplicationRuntimeInspectionAdapter의 status 처리에 필요한 업무 로직을 수행한다. */
    private String status(Integer ready, Integer desired) { return value(ready) >= value(desired) ? "HEALTHY" : "DEGRADED"; }

    /** Fabric8ApplicationRuntimeInspectionAdapter의 client 처리에 필요한 업무 로직을 수행한다. */
