...
class ApplicationEndpointCollectorTest {
    /** ApplicationEndpointCollectorTest의 collectsReadyTlsIngressOwnedByHelmAnnotation 처리의 핵심 작업 흐름을 실행한다. */
    @Test
    void collectsReadyTlsIngressOwnedByHelmAnnotation() {
        ...
        assertThat(endpoints).singleElement().satisfies(endpoint -> {
            ...
            assertThat(endpoint.address()).isEqualTo("web.example.test");
            assertThat(endpoint.port()).isEqualTo(443);
            assertThat(endpoint.accessScope()).isEqualTo("EXTERNAL_DOMAIN");
        });
    }

    /** ApplicationEndpointCollectorTest의 collectsHttpRouteAndMapsAcceptedConditions 처리의 핵심 작업 흐름을 실행한다. */
    @Test
    void collectsHttpRouteAndMapsAcceptedConditions() {
        ...
        assertThat(endpoints).singleElement().satisfies(endpoint -> {
            ...
            assertThat(endpoint.status()).isEqualTo("READY");
            assertThat(endpoint.address()).isEqualTo("sample.example.test");
