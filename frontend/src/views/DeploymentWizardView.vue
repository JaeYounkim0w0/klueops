...
import { api, type ClusterResponse, type DeploymentPlanResponse } from '@/api/client';
...
const message = ref('');
const target = reactive({ clusterId: '', namespace: 'default', releaseName: '', exposureType: 'NONE', hostname: '' });
...
async function preview(): Promise<void> {
  ...
  try {
    plan.value = await api.createDeploymentPlan({ tenantId: tenancy.currentTenantId, clusterId: target.clusterId,
      ...
      namespace: target.namespace, releaseName: target.releaseName, exposureType: target.exposureType, hostname: target.hostname || undefined });
    confirmation.value = '';
...
<template>
  <section class="page delivery-page">
    <header class="delivery-hero compact"><div><span class="delivery-eyebrow">DEPLOYMENT WIZARD</span><h1>{{ chartName }} 배포</h1><p>대상과 노출 방식을 선택한 뒤 렌더 결과를 검토합니다.</p></div><RouterLink to="/applications/library" class="secondary-button">나가기</RouterLink></header>
    ...
    <div v-if="!plan" class="target-layout"><section class="delivery-panel"><header><span class="delivery-eyebrow">TARGET</span><h2>Cluster와 Namespace</h2></header><div class="delivery-form-grid"><label class="wide">Cluster<select v-model="target.clusterId" required><option disabled value="">Cluster 선택</option><option v-for="cluster in clusters" :key="cluster.id" :value="cluster.id">{{ cluster.name }} · {{ cluster.environment }}</option></select></label><label>Namespace<input v-model="target.namespace" required /></label><label>Release name<input v-model="target.releaseName" required /></label></div></section><section class="delivery-panel"><header><span class="delivery-eyebrow">EXPOSURE</span><h2>접근 경로</h2></header><div class="exposure-options"><label :class="{ selected: target.exposureType === 'NONE' }"><input v-model="target.exposureType" type="radio" value="NONE" /><strong>Cluster 내부</strong><small>Chart 기본 Service만 사용</small></label><label :class="{ selected: target.exposureType === 'HTTP_ROUTE' }"><input v-model="target.exposureType" type="radio" value="HTTP_ROUTE" /><strong>HTTPRoute</strong><small>Gateway API 기반 도메인 연결</small></label></div><label v-if="target.exposureType === 'HTTP_ROUTE'" class="delivery-field">Hostname<input v-model="target.hostname" placeholder="nginx.cluster.example.com" /></label></section><footer class="wizard-footer"><span>Preview는 Cluster를 변경하지 않습니다.</span><button class="primary-button" type="button" :disabled="planning || !target.clusterId" @click="preview">{{ planning ? 'Helm 렌더링 중…' : 'Preview 생성' }} <i class="pi pi-arrow-right"></i></button></footer></div>
