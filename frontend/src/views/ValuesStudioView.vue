...
async function suggestValues(): Promise<void> {
  ...
  } catch (error) { message.value = error instanceof Error ? error.message : 'AI Values 제안을 생성하지 못했습니다.'; }
...
<template>
  <section class="page delivery-page">
    ...
    <div v-if="assistantOpen" class="delivery-modal-backdrop" @click.self="assistantOpen = false"><section class="delivery-modal" role="dialog" aria-modal="true"><header><div><span class="delivery-eyebrow">REVIEWABLE AI SUGGESTION</span><h2>Custom Values 도우미</h2><p>Secret 유사 key는 마스킹한 뒤 Tenant의 HELM_VALUES Provider로 전송합니다. 결과는 자동 저장되지 않습니다.</p></div><button class="icon-button" type="button" @click="assistantOpen = false"><i class="pi pi-times"></i></button></header><label class="delivery-field">요청 내용<textarea v-model="aiInstruction" maxlength="2000" placeholder="예: replica를 3개로 늘리고 resources requests/limits를 보수적으로 설정해줘"></textarea></label><div class="delivery-assistant-actions"><button class="secondary-button" type="button" :disabled="suggesting || !aiInstruction.trim()" @click="suggestValues">{{ suggesting ? '제안 생성 중…' : '제안 생성' }}</button></div><label v-if="aiSuggestion" class="delivery-field">검토할 제안<textarea v-model="aiSuggestion" class="suggestion-preview" spellcheck="false"></textarea></label><footer><button class="secondary-button" type="button" @click="assistantOpen = false">취소</button><button class="primary-button" type="button" :disabled="!aiSuggestion" @click="applySuggestion">검토한 제안을 편집기에 적용</button></footer></section></div>
  </section>
</template>
