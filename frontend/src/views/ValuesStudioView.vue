...
/** suggestValues 처리에 필요한 화면 또는 업무 로직을 수행한다. */
async function suggestValues(): Promise<void> {
  ...
  try {
    const result = await api.suggestValues({
      tenantId: tenancy.currentTenantId,
      chartVersionId: chartVersionId.value,
      ...
      <section class="delivery-modal" role="dialog" aria-modal="true">
        <header>
          <div>
            ...
            <p>
              선택한 Chart의 제공사·Chart/App 버전·기본 Values·Schema를 기준으로
              제안하고, 실제 Helm 렌더링을 통과한 결과만 표시합니다. Secret 값은
              AI에 보내지 않습니다.
            </p>
          </div>
          <button
        ...
        <div class="delivery-assistant-actions">
          <button
            class="secondary-button"
            type="button"
            :disabled="suggesting || !aiInstruction.trim()"
            @click="suggestValues"
          >
            {{ suggesting ? "Chart 계약 검증 중…" : "제안 생성" }}
          </button>
        </div>
        <div v-if="aiResult" class="assistant-validation-summary">
          <span class="status-dot success">Helm 검증 완료</span
          ><span
            >{{ aiResult.chartName }} ·
            ...
            ></span
          ><small
            >{{ aiResult.promptVersion }} · {{ aiResult.attempts }}회 생성 ·
