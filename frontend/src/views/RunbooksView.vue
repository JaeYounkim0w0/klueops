...
<template>
  <section class="page runbook-library-page">
    ...
    <div v-if="editorOpen" class="modal-backdrop" @click.self="editorOpen = false"><section class="modal-panel runbook-editor-modal" role="dialog" aria-modal="true" aria-labelledby="runbook-editor-title"><header class="modal-header"><div><span class="page-eyebrow">VERSIONED RUNBOOK</span><h2 id="runbook-editor-title">{{ editingId ? 'Runbook 편집' : 'Runbook 작성' }}</h2><p>명령의 목적과 기대 결과를 분리해 다음 운영자도 안전하게 판단할 수 있게 합니다.</p></div><button class="icon-button" type="button" title="닫기" aria-label="닫기" @click="editorOpen = false"><i class="pi pi-times"></i></button></header><form class="runbook-editor-form" @submit.prevent="saveRunbook">
    ...
    <div v-if="versionsOpen" class="modal-backdrop" @click.self="versionsOpen = false"><section class="modal-panel runbook-version-modal" role="dialog" aria-modal="true" aria-labelledby="runbook-version-title"><header class="modal-header"><div><span class="page-eyebrow">IMMUTABLE HISTORY</span><h2 id="runbook-version-title">버전 이력</h2><p>{{ selectedRunbook?.title }}</p></div><button class="icon-button" type="button" title="닫기" aria-label="닫기" @click="versionsOpen = false"><i class="pi pi-times"></i></button></header><div class="runbook-version-list"><article v-for="version in versions" :key="version.id"><span>v{{ version.version }}</span><div><strong>{{ version.changeNote || '변경 메모 없음' }}</strong><small>{{ version.createdBy }} · {{ formatTimestamp(version.createdAt) }}</small></div><button class="secondary-button compact" type="button" @click="restore(version)"><i class="pi pi-replay"></i><span>복원</span></button></article></div></section></div>
  </section>
</template>
